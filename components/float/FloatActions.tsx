...
export function FloatActions({ rows }: { rows: FloatRow[] }) {
  ...
  const exportCsv = () => {
    try {
      const header = ["Ticker", "Float on chain", "Locked in pools", "Share gripped (%)", "Pools"];
      ...
      showFeedback(setExportFeedback, exportTimer, { kind: "success", message: "Downloaded!" });
    } catch {
      showFeedback(setExportFeedback, exportTimer, { kind: "error", message: "Download failed" });
    }
  };
  ...
  const copyBoard = async () => {
    try {
      ...
      const text = [
        ["Ticker", "Float on chain", "Locked in pools", "Share gripped (%)", "Pools"].join("\t"),
        ...rows.map((row) => [
          row.ticker ?? "",
          numberOrBlank(row.floatOnChain),
          numberOrBlank(row.lockedInPools),
  ...
  return (
    <div className="flex flex-shrink-0 items-start gap-[7px]" style={{ marginLeft: "auto" }}>
      <div className="flex flex-col items-end gap-[3px]">
        <button
          type="button"
          onClick={exportCsv}
          className="rounded-[4px] border border-line2 bg-transparent px-[13px] font-sans text-[12px] font-medium text-fg2 transition-colors hover:border-line hover:text-fg"
          style={{ height: 29 }}
        >
          {exportFeedback?.message ?? "Export CSV"}
        </button>
        <span className={`h-[10px] font-mono text-[9px] ${exportFeedback?.kind === "error" ? "text-down" : "text-fg3"}`} aria-live="polite">
      ...
      <div className="flex flex-col items-end gap-[3px]">
        <button
          type="button"
          onClick={copyBoard}
          className="rounded-[4px] border-0 bg-fg px-[13px] font-sans text-[12px] font-medium text-bg transition-opacity hover:opacity-90"
          style={{ height: 29 }}
        >
